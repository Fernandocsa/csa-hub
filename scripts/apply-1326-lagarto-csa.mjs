/**
 * Apply Série D #1326 Lagarto 1x1 CSA (2026-06-21) from user sumula.
 * Paulo Barreto (Barretão); referee Matheus de Moraes Silva.
 */
...
async function ensureReferee(name) {
  ...
  const soft = all.rows.filter((r) => {
    const n = norm(r.name);
    return n.includes("matheus") && n.includes("moraes");
  });
...
try {
  ...
  const referee = await ensureReferee("Matheus de Moraes Silva");
  ...
  const bench = [
    ...
    { shirt: 80, name: "Everton Heleno", id: 104, position: "Meia" },
  ...
  const cards = [
    { id: 477, name: "Yago Oliveira", type: "yellow", minute: 66, injury: null },
    { id: 462, name: "Ailton Santos", type: "yellow", minute: 84, injury: null },
  ];

  // Coverage: Lima→Everton Heleno, Dudu→Melo; Bigode→Ronaldo; Rian→Gustavo; Ailton→Kaike
  const subs = [
    {
      outId: 476,
      out: "Lucas Lima",
  ...
  await client.query(
    `UPDATE matches SET
       ...
       home_away='away'
     WHERE id=$1`,
    [MATCH_ID, STADIUM_ID, MANAGER_ID, referee.id, 432, "Camacho"],
  );
