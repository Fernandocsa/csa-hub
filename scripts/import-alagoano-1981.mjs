/**
 * Import Campeonato Alagoano 1981 (36 jogos CSA).
 * - Técnico: Valmir Louruz (fonte: Walmir Louruz)
 * - Título antecipado em 22/11/1981 (Penedense 1x3)
 * - Soma dos placares da fonte: J36 V27 E6 D3 GP79 GC22
 *   (cabeçalho final da fonte cita V28 E6 D2 GP72 GC20 — inconsistente com as fases)
 * - Gols contra: ownGoalDirection for|against + own_goals_for_count
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1981-alagoano.mjs";
...
const CANON = {
  ...
  "jorge luis": "Jorge Luiz",
  "jorge luís": "Jorge Luiz",
  nilson: "Nílson",
  nílson: "Nílson",
  "luis felipe": "Luís Felipe",
  "luís felipe": "Luís Felipe",
  "luiz felipe": "Luís Felipe",
  josenilton: "Josenílton",
  josenílton: "Josenílton",
  "osmar barão": "Osmar Barão",
  "osmar barao": "Osmar Barão",
  "ze luiz ii": "Zé Luiz II",
  "zé luiz ii": "Zé Luiz II",
  brasilia: "Brasília",
  brasília: "Brasília",
  decio: "Décio",
  décio: "Décio",
...
const CREATE_META = {
  "jorge luiz": { position: "Meia", forceCreate: true },
  parana: { position: "Atacante", forceCreate: true },
  paraná: { position: "Atacante", forceCreate: true },
  decio: { position: "Goleiro", forceCreate: true },
  décio: { position: "Goleiro", forceCreate: true },
  "ze luiz ii": { position: "Atacante", forceCreate: true },
  "zé luiz ii": { position: "Atacante", forceCreate: true },
  poty: { position: "Atacante", forceCreate: true },
  "ronaldo alves": { position: "Zagueiro", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (hit) return hit;
  if (
    norm(canon).includes("louruz") ||
    norm(canon).includes("walmir") ||
    norm(canon).includes("valmir")
  ) {
    const v = all.find(
      (m) =>
        norm(m.name).includes("louruz") ||
        norm(m.name).includes("walmir") ||
        norm(m.name).includes("valmir"),
    );
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log(
    "soma fonte: J36 V27 E6 D3 GP79 GC22 | cabeçalho final cita V28 E6 D2 GP72 GC20 (inconsistente)",
  );
  console.log("OK Alagoano 1981");
} catch (e) {
