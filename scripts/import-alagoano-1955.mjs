/**
 * Import Campeonato Alagoano 1955 (19 jogos CSA).
 * Documento: "Campeonato da Capital — Edição 1955" (só Maceió) — gravado como Alagoano.
 * Técnico: Segismundo Cerqueira (4 jogos no início do 1º turno).
 * Season=1955 inclusive 2º turno jan–mai/1956.
 * Oficial: J19 V16 E2 D1 GP64 GC14.
 * CSA campeão 1º e 2º turnos; título antecipado 24/05/1956; Ferroviário vice.
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1955-alagoano.mjs";
...
const CANON = {
  ...
  "nelson veragio": "Nélson Verágio",
  "nélson verágio": "Nélson Verágio",
  "jose aleixo": "José Aleixo",
  "josé aleixo": "José Aleixo",
  "rosalvo peixoto": "Rosalvo Peixoto",
  "adalberto silva": "Adalberto Silva",
  "segismundo cerqueira": "Segismundo Cerqueira",
};
...
const FORCE_ID = {
  ...
  eugênio: 2216,
  oscarzinho: 2213,
  nivaldo: 2204, // zagueiro (não goleiro #2128)
};
...
const CREATE_META = {
  epaminondas: { name: "Epaminondas", position: "Goleiro", forceCreate: true },
  ...
  bore: { name: "Boré", position: "Atacante", forceCreate: true },
  boré: { name: "Boré", position: "Atacante", forceCreate: true },
  vetinho: { name: "Vetinho", position: "Meia", forceCreate: true },
  berenaldo: { name: "Berenaldo", position: "Meia", forceCreate: true },
  bem: { name: "Bem", position: "Zagueiro", forceCreate: true },
  cao: { name: "Cão", position: "Atacante", forceCreate: true },
  cão: { name: "Cão", position: "Atacante", forceCreate: true },
  terto: { name: "Terto", position: "Goleiro", forceCreate: true },
  noe: { name: "Noé", position: "Zagueiro", forceCreate: true },
  noé: { name: "Noé", position: "Zagueiro", forceCreate: true },
  pitota: { name: "Pitota", position: "Meia", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (norm(canon).includes("segismundo")) {
    const v = all.find((m) => norm(m.name).includes("segismundo"));
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  await client.query(
    `UPDATE season_competition_stats
     SET classification='1º', is_champion=true, stats_recalculated_at=now()
  ...
  console.log("oficial esperado: J19 V16 E2 D1 GP64 GC14");
  console.log("OK Alagoano 1955");
} catch (e) {
