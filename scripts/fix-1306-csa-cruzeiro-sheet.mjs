/**
 * Fix CSA sheet for #1306 CSA 2x0 Cruzeiro-AL (2026-02-07) from user sumula.
 * Keeps opponent lineups/cards/subs. Sets shirts, positions, goals+assist, cards, CSA subs.
 */
...
const bench = [
  ...
  { shirt: 13, name: "Wesley", id: 308, position: "Volante" },
...
const cards = [
  { id: 468, name: "Ronaldo Mendes", type: "yellow", minute: 51 },
  { id: 453, name: "Kayllan", type: "yellow", minute: 85 },
];
...
try {
  ...
  await client.query(
    `UPDATE matches SET
       ...
       scorers=$4
     WHERE id=$1`,
    [MATCH_ID, MANAGER_ID, REFEREE_ID, "Buba, Ronaldo Mendes"],
  );

  // Keep opponent rows; rewrite CSA sheet + CSA events
  await client.query(
  ...
  // Normalize Fabricio spelling on player row if needed
  await client.query(
    `UPDATE players SET name='Fabrício Bigode' WHERE id=463 AND name <> 'Fabrício Bigode'`,
  );
  await client.query(
    `UPDATE players SET name='Samuel Reis' WHERE id=458 AND name <> 'Samuel Reis'`,
  );
