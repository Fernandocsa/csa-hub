/**
 * Import Campeonato Alagoano 1976 (28 jogos CSA).
 * - CSA vice-campeão; CRB campeão (jogo extra 08/08)
 * - Técnicos: Wassil Barbosa (até 06/06) / Hélio Miranda (08/08)
 * - Soma: J28 V16 E8 D4 GP67 GC18 (tabela final fonte GP72 GC19)
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1976-alagoano.mjs";
...
try {
  ...
  await client.query(
    `UPDATE season_competition_stats
     SET classification = '2º', is_champion = false
     WHERE season = $1 AND competition_id = $2`,
  ...
  const expected = { games: 28, wins: 16, draws: 8, losses: 4, goals_for: 67, goals_against: 18 };
  const ok =
  ...
  console.log(
    "soma placares: J28 V16 E8 D4 GP67 GC18 | tabela final fonte GP72 GC19",
  );
  if (!ok) {
    throw new Error(
