/**
 * Import Campeonato Alagoano 1962 (14 jogos CSA).
 * - Técnicos: Eduardo Montenegro (1º turno) / Ivon Cordeiro (1x no 2º)
 * - Season=1962 inclusive jogos jan–mai/1963 dos turnos
 * - Oficial extraído: J14 V5 E3 D6 GP32 GC27
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1962-alagoano.mjs";
...
const CANON = {
  ...
  "ze chaves": "Zé Chaves",
  "zé chaves": "Zé Chaves",
  "batista cortes": "Batista Cortez",
  "batista cortês": "Batista Cortez",
  "claudio regis": "Cláudio Régis",
  "cláudio régis": "Cláudio Régis",
  "cláudio regis": "Cláudio Régis",
  "eduardo montenegro": "Eduardo Montenegro",
  "ivon cordeiro": "Ivon Cordeiro",
  "glenio guimaraes": "Glênio Guimarães",
  "glênio guimarães": "Glênio Guimarães",
};
...
const CREATE_META = {
  biu: { name: "Biu", position: "Atacante", forceCreate: true },
  bil: { name: "Bil", position: "Atacante", forceCreate: true },
  farias: { name: "Farias", position: "Zagueiro", forceCreate: true },
  joaozinho: { name: "Joãozinho", position: "Lateral", forceCreate: true },
  joãozinho: { name: "Joãozinho", position: "Lateral", forceCreate: true },
  gereba: { name: "Gereba", position: "Goleiro", forceCreate: true },
  "ze chaves": { name: "Zé Chaves", position: "Meia", forceCreate: true },
  "zé chaves": { name: "Zé Chaves", position: "Meia", forceCreate: true },
  charles: { name: "Charles", position: "Meia", forceCreate: true },
  neco: { name: "Neco", position: "Atacante", forceCreate: true },
  // atacante 1962 — distinto do goleiro Ademir #2193
  ademir: { name: "Ademir", position: "Atacante", forceCreate: true },
};
...
async function ensureReferee(name) {
  ...
  if (hit) return hit;
  if (norm(canon).includes("walfrido")) {
    const w = all.find((r) => norm(r.name).includes("walfrido"));
    if (w) return w;
  }
...
try {
  ...
  console.log("oficial esperado (Alagoano): J14 V5 E3 D6 GP32 GC27");
  console.log("OK Alagoano 1962");
} catch (e) {
