/**
 * Import Campeonato Alagoano 1979 (34 jogos CSA).
 * - Técnicos: Wassil/Vassil Barbosa → Hélio Miranda → Zé Galego (interino)
 * - CSA 4º (regulamento: superturno define o top-4; CRB campeão, ASA vice)
 * - Soma dos placares: J34 V19 E6 D9 GP67 GC33
 *   (tabela final da fonte cita GC32; superturno lista GC11 vs soma 12)
 * - Gols contra: ownGoalDirection for|against + own_goals_for_count
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1979-alagoano.mjs";
...
const CANON = {
  "vassil barbosa": "Wassil Barbosa",
  "wassil barbosa": "Wassil Barbosa",
  "helio miranda": "Hélio Miranda",
  "hélio miranda": "Hélio Miranda",
  "ze galego": "Zé Galego",
  "zé galego": "Zé Galego",
  "luis carlos": "Luís Carlos",
  "luís carlos": "Luís Carlos",
  "luiz carlos": "Luís Carlos",
  "ze luiz": "Zé Luiz",
  "zé luiz": "Zé Luiz",
  "ze luis": "Zé Luiz",
  "zé luís": "Zé Luiz",
...
const CREATE_META = {
  "ze luiz": { position: "Zagueiro", forceCreate: true },
  "zé luiz": { position: "Zagueiro", forceCreate: true },
  evaristo: { position: "Lateral", forceCreate: true },
  alex: { position: "Volante", forceCreate: true },
  radar: { position: "Atacante", forceCreate: true },
  belisco: { position: "Volante", forceCreate: true },
  samuel: { position: "Goleiro", forceCreate: true },
  valvir: { position: "Goleiro", forceCreate: true },
  nego: { position: "Goleiro", forceCreate: true },
  helio: { position: "Lateral", forceCreate: true },
  hélio: { position: "Lateral", forceCreate: true },
  claudio: { position: "Atacante", forceCreate: true },
  cláudio: { position: "Atacante", forceCreate: true },
  cardoso: { position: "Lateral", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (hit) return hit;
  if (norm(canon).includes("wassil") || norm(canon).includes("vassil") || norm(canon).includes("barbosa")) {
    const v = all.find(
      (m) =>
        norm(m.name).includes("wassil") ||
        norm(m.name).includes("vassil") ||
        (norm(m.name).includes("barbosa") && norm(m.name).includes("wassil")),
    );
    if (v) return v;
    const w = all.find((m) => norm(m.name) === "wassil barbosa");
    if (w) return w;
  }
  if (norm(canon).includes("miranda")) {
    const v = all.find((m) => norm(m.name).includes("miranda"));
    if (v) return v;
  }
  if (norm(canon).includes("galego")) {
    const v = all.find((m) => norm(m.name).includes("galego"));
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log(
    "soma placares: J34 V19 E6 D9 GP67 GC33 | tabela final fonte GC32 (superturno 11 vs 12)",
  );
  console.log("OK Alagoano 1979");
} catch (e) {
