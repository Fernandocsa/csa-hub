/**
 * Import CSA — Torneio Seletivo Taça de Prata 1984 (4 jogos) + súmulas CSA.
 */
...
import {
  ...
  COMPETITION_TYPE,
  GAMES,
} from "./data/season-1984-torneio-seletivo-taca-de-prata.mjs";
...
const CREATE_META = {
  carlinhos: { name: "Carlinhos", position: "Meia", forceNew: true },
  gera: { name: "Gera", position: "Atacante", forceNew: true },
  jorginho: { name: "Jorginho", position: "Meia", forceNew: true },
};
...
async function ensureCsaPlayer(raw) {
  ...
  if (FORCE_ID[key]) {
    ...
    // Display Josenílton for accented DB name
...
async function ensureCompetition() {
  let { rows } = await client.query(`SELECT id, name FROM competitions WHERE name=$1`, [
    COMPETITION_NAME,
  ]);
...
async function ensureOpponent(name) {
  ...
  if (rows[0]) return rows[0];
  const state = name.match(/-([A-Z]{2})$/)?.[1] ?? null;
  const ins = await client.query(
...
async function ensureStadium(name) {
  ...
  if (soft) return soft;
  if (norm(name).includes("coaracy")) {
    const c = all.find((s) => norm(s.name).includes("coaracy"));
    if (c) return c;
  }
...
async function ensureManager(name) {
  ...
  if (hit) return hit;
  const soft = all.find((m) => norm(m.name).includes(norm(name)));
  if (soft) return soft;
  const ins = await client.query(
  ...
  for (const g of GAMES) {
    ...
    for (const goal of g.oppGoals ?? []) {
      await client.query(
        ...
        [matchId, goal.name, goal.minute ?? 0, !!goal.penalty],
      );
    }
