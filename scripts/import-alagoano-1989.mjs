/**
 * Import Campeonato Alagoano 1989 (39 jogos oficiais) + fichas/gols quando informados.
 * - CSA x Capelense 07/06/1989 anulada (falta de energia) — omitida; importa a remarcada 08/06
 * - Público/renda gravados quando a fonte informa (casa ou fora)
 * - Técnico: Pompeia em toda a temporada
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1989-alagoano.mjs";
...
/** IDs confirmados da época / já com stats 1989–1990. */
const FORCE_ID = {
  chico: 551,
  ivan: 541,
  zezinho: 536,
...
/** forceCreate: evita colidir com homônimos de outras épocas. */
const CREATE_META = {
  jedson: { position: "Goleiro", forceCreate: true },
  jédson: { position: "Goleiro", forceCreate: true },
  irlis: { position: "Zagueiro", forceCreate: true },
  írlis: { position: "Zagueiro", forceCreate: true },
  boza: { position: "Atacante", forceCreate: true },
  roberval: { position: "Zagueiro", forceCreate: true },
  nilton: { position: "Meia", forceCreate: true },
  nílton: { position: "Meia", forceCreate: true },
  "carlinhos paulista": { position: "Atacante", forceCreate: true },
  macedo: { position: "Atacante", forceCreate: true },
  luque: { position: "Atacante", forceCreate: true },
  wellington: { position: "Lateral", forceCreate: true },
  osvaldo: { position: "Atacante", forceCreate: true },
  geraldo: { position: "Meia", forceCreate: true },
  ricardo: { position: "Atacante", forceCreate: true },
};
...
try {
  ...
  for (const g of GAMES) {
    ...
    const ownNames =
      g.goals?.filter((x) => x.ownGoal).map((x) => `${x.name} (gc)`) ?? [];
    const scorers =
      csaGoalNames.length || ownNames.length
        ? [...csaGoalNames, ...ownNames].join(", ")
        : null;
  ...
  console.log(
    "fonte final: J38 V21 E08 D09 GP54 GC25 (PG50 c/ bônus de turnos); matches importados excluem anulado 07/06",
  );
  console.log("OK Alagoano 1989");
} catch (e) {
