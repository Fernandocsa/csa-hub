/**
 * Apply Série D #1321 Juazeirense 0x1 CSA (2026-05-10) from user sumula.
 * Adauto Moraes; referee Luciano da Silva Miranda Filho.
 */
...
async function ensureReferee(name) {
  ...
  const soft = all.rows.filter((r) => {
    const n = norm(r.name);
    return n.includes("luciano") && n.includes("miranda");
  });
...
try {
  ...
  const referee = await ensureReferee("Luciano da Silva Miranda Filho");
  ...
  const starters = [
    ...
    { shirt: 3, name: "Ramon Batista", id: 435, position: "Volante" },
    ...
    { shirt: 5, name: "Fabrício Bigode", id: 463, position: "Volante" },
  ...
  const bench = [
    ...
    { shirt: 13, name: "Gustavo", id: 433, position: "Lateral Esquerdo" },
    ...
    { shirt: 25, name: "Vitinho", id: 474, position: "Centroavante" },
    { shirt: 9, name: "Lucas Lima", id: 476, position: "Atacante" },
    { shirt: 7, name: "Lucas Silva", id: 71, position: "Atacante" },
  ...
  const cards = [
    { id: 460, name: "Rian Santana", type: "yellow", minute: 23, injury: null },
    {
      id: 463,
      name: "Fabrício Bigode",
      type: "yellow",
      minute: 42,
      injury: null,
    },
  ...
  // 60' outs (list order): Ramon, Dudu, Rian → Wesley, Lucas Lima, Lucas Silva
  const subs = [
    {
      outId: 435,
      out: "Ramon Batista",
  ...
  await client.query(
    `UPDATE matches SET
       ...
       home_away='away'
     WHERE id=$1`,
    [MATCH_ID, STADIUM_ID, MANAGER_ID, referee.id, "Lucas Lima"],
  );
  ...
  const gl = await client.query(
    `SELECT minute, injury_time_minute, scorer_name, assist_name FROM match_goals
