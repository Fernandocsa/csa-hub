/**
 * Import Taça de Ouro 1976 (Brasileiro Série A) — 12 jogos CSA.
 * - 1ª fase + Repescagem
 * - Oficial: J12 V2 E5 D5 GP14 GC19
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1976-taca-ouro.mjs";
...
try {
  ...
  const expected = { games: 12, wins: 2, draws: 5, losses: 5, goals_for: 14, goals_against: 19 };
  const ok =
  ...
  console.log("seasonStats (Taça de Ouro)", seasonStats);
  console.log("rosterPlayers", rosterN);
  console.log("oficial: J12 V2 E5 D5 GP14 GC19");
  if (!ok) {
    throw new Error(
      `Stats mismatch: got ${JSON.stringify(seasonStats)} expected ${JSON.stringify(expected)}`,
    );
  }
  console.log("OK Taça de Ouro 1976");
} catch (e) {
