/**
 * Import Campeonato Alagoano 1984 (49 jogos CSA; 48 na contagem oficial).
 * - Caso doping 05/06: placar 2x1 em campo; result=loss (pontos ao ASA no STJD)
 * - Decisão 13/06 anulada: importada e excluída de season_competition_stats
 * - Técnico: Valdemar Carabina
 * - Gols contra: ownGoalDirection for|against + own_goals_for_count
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1984-alagoano.mjs";
...
const CANON = {
  ...
  luizão: "Luizão",
  luisão: "Luizão",
  "luiz augusto": "Luiz Augusto",
  "luís augusto": "Luiz Augusto",
  "luis augusto": "Luiz Augusto",
  frazao: "Frazão",
  frazão: "Frazão",
...
const CREATE_META = {
  "ze carlos": { position: "Meia", forceCreate: true },
  "zé carlos": { position: "Meia", forceCreate: true },
  noronha: { position: "Meia", forceCreate: true },
  dario: { position: "Atacante", forceCreate: true },
  careca: { position: "Lateral", forceCreate: true },
  "luiz augusto": { position: "Zagueiro", forceCreate: true },
  nene: { position: "Atacante", forceCreate: true },
  nenê: { position: "Atacante", forceCreate: true },
  cardoso: { position: "Atacante", forceCreate: true },
  laerte: { position: "Meia", forceCreate: true },
  silva: { position: "Volante", forceCreate: true },
  carlos: { position: "Meia", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (hit) return hit;
  if (norm(canon).includes("carabina") || norm(canon).includes("valdemar")) {
    const v = all.find(
      (m) => norm(m.name).includes("carabina") || norm(m.name).includes("valdemar"),
    );
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log("seasonStats (oficial, exclui anulado)", seasonStats);
  console.log("rosterPlayers", rosterN);
  console.log(
    "fonte oficial: J48 V31 E11 D06 GP84 GC17 | PG79 c/ bônus não está em matches",
  );
  console.log("OK Alagoano 1984");
} catch (e) {
