/**
 * Apply Série D #1320 CSA 2x0 Juazeirense (2026-05-02) from user sumula.
 * Rei Pelé; referee Leonilson Fernandes Trigueiro Filho.
 */
...
async function ensureReferee(name) {
  ...
  const soft = all.rows.filter((r) => {
    const n = norm(r.name);
    return n.includes("leonilson") || n.includes("trigueiro");
  });
...
try {
  ...
  const referee = await ensureReferee("Leonilson Fernandes Trigueiro Filho");
  ...
  const cards = [
    { id: 432, name: "Camacho", type: "yellow", minute: 52, injury: null },
    { id: 466, name: "Marcos Ytalo", type: "yellow", minute: 89, injury: null },
  ];
