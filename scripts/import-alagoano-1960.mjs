/**
 * Import Campeonato Alagoano 1960 (10 jogos CSA; 1 com placar unknown / data year-only).
 * - Técnico: Madalena (quando informado)
 * - Season=1960 inclusive jogo do título 22/01/1961
 * - Oficial com placar: J9 V7 E1 D1 GP23 GC10 (inclui 1 W.O. 1x0)
 * - +1 Ferroviário unknown (1960-01-01 year-only)
 */
...
import {
  ...
  SEASON,
  GAMES,
} from "./data/season-1960-alagoano.mjs";
...
const CANON = {
  ...
  "agostin ferrapeira": "Agostín Ferrapeira",
  "agostín ferrapeira": "Agostín Ferrapeira",
  madalena: "Madalena",
};
...
const FORCE_ID = {
  moacir: 1028, // goleiro b1930 (era); não #579 b1955
  nenem: 1039, // Neném meia/era 1959
  neném: 1039,
  "paulo santos": 1069,
  carlinhos: 734, // meia
  piolho: 1076,
  ...
  bil: 2196,
  biu: 2197,
  italo: 903, // meia b1935 era 1959–61
  ítalo: 903,
  clovis: 526,
  clóvis: 526,
  deda: 775, // meia 1941
...
const CREATE_META = {
  "bil wilson": { name: "Bil Wilson", position: "Atacante", forceCreate: true },
  eromir: { name: "Eromir", position: "Atacante", forceCreate: true },
  giba: { name: "Giba", position: "Atacante", forceCreate: true },
  casado: { name: "Casado", position: "Goleiro", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (norm(canon).includes("madalena")) {
    const v = all.find((m) => norm(m.name).includes("madalena"));
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log("oficial esperado (com placar): J9 V7 E1 D1 GP23 GC10 + 1 unknown Ferroviário");
  console.log("OK Alagoano 1960");
} catch (e) {
