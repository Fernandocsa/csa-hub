/**
 * Apply Série D #1319 CSA 1x1 CSE (2026-04-25) from user sumula.
 * Rei Pelé; referee Fábio Augusto Santos Sá Júnior.
 */
...
async function ensureReferee(name) {
  ...
  const soft = all.rows.filter((r) => {
    const n = norm(r.name);
    return (
      n.includes("fabio augusto") &&
      (n.includes("sa junior") || n.includes("santos sa"))
    );
  });
...
try {
  ...
  const referee = await ensureReferee("Fábio Augusto Santos Sá Júnior");
  ...
  const bench = [
    ...
    { shirt: 15, name: "Luiz Guilherme", id: 440, position: "Meia Ofensivo" },
    { shirt: 22, name: "Matheus Sacramento", id: 452, position: "Centroavante" },
    { shirt: 26, name: "Vitinho", id: 474, position: "Centroavante" },
  ...
  // Sacramento "R 61'" skipped — entered only at 90'; Melo already has 61'.
  const cards = [
    {
      id: 463,
    ...
    { id: 483, name: "Matheus Melo", type: "yellow", minute: 61, injury: null },
    { id: 462, name: "Ailton Santos", type: "yellow", minute: 74, injury: null },
  ];
