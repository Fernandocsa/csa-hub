/**
 * Import Campeonato Alagoano 1975 (25 jogos; 24 na contagem oficial).
 * - CSA campeão dos 3 turnos (tricampeão de turnos) + título geral
 * - Técnico: Laerte Dória
 * - 25/05 vs Guarany: jogo anulado (falta de energia); remarcado 04/06
 * - Oficial: J24 V21 E2 D1 GP66 GC7
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1975-alagoano.mjs";
...
const CANON = {
  ...
  "sergio galocha": "Sérgio Galocha",
  "sérgio galocha": "Sérgio Galocha",
...
const CREATE_META = {
  rafael: { position: "Goleiro", forceCreate: true },
  espinoza: { position: "Lateral", forceCreate: true },
  ferretti: { position: "Atacante", forceCreate: true },
  major: { position: "Atacante", forceCreate: true },
  mauricio: { position: "Volante", forceCreate: true },
  maurício: { position: "Volante", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (hit) return hit;
  if (norm(canon).includes("laerte") || norm(canon).includes("doria")) {
    const v = all.find(
      (m) => norm(m.name).includes("laerte") && norm(m.name).includes("doria"),
    );
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log(
    "oficial (exclui anulado 25/05): J24 V21 E2 D1 GP66 GC7 | CSA tricampeão de turnos",
  );
  console.log("OK Alagoano 1975");
} catch (e) {
