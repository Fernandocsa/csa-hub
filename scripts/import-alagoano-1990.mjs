/**
 * Import Campeonato Alagoano 1990 (30 jogos) + fichas/gols quando informados.
 * - ASA x CSA do 2º turno omitido (não realizado)
 * - Público/renda gravados quando a fonte informa (casa ou fora)
 * - Gol contra Marcos Silva → CSA (own_goal_direction=for)
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1990-alagoano.mjs";
...
const CREATE_META = {
  wilson: { position: "Goleiro", birth_year: null },
  tie: { position: "Lateral Direito", birth_year: null },
  tiê: { position: "Lateral Direito", birth_year: null },
  "carlos silva": { position: "Meia", birth_year: null },
  "luis carlos": { position: "Atacante", birth_year: null },
  "luís carlos": { position: "Atacante", birth_year: null },
  "luiz carlos": { position: "Atacante", birth_year: null },
};
...
async function ensurePlayer(raw) {
  ...
  let { rows } = await client.query(`SELECT id, name, position FROM players WHERE name=$1`, [
    name,
  ]);
  // Evita colidir Wilson atacante (#539) com o goleiro de 1990
  if (
    rows[0] &&
    meta?.position &&
    rows[0].position &&
    rows[0].position !== meta.position &&
    key === "wilson"
  ) {
    rows = [];
  }
  if (!rows[0]) {
    const pos = meta?.position ?? null;
    const by = meta?.birth_year ?? null;
    const insertName = key === "wilson" && meta?.position === "Goleiro" ? "Wilson" : name;
    // se já existe outro Wilson, cria com sufixo interno e display Wilson nas fichas
    let ins;
    try {
  ...
  playerCache.set(key, { id: rows[0].id, name: key === "wilson" ? "Wilson" : rows[0].name });
...
try {
  ...
  console.log(
    "expected: J30 V19 E9 D2 GP45 GC11 | PG47 (com bônus dos turnos não está em matches)",
  );
  console.log("OK Alagoano 1990");
} catch (e) {
