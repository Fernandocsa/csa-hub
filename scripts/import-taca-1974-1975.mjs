/**
 * Import Taça de Ouro 1974–1975 (Brasileiro Série A) — jogos do CSA.
 * Usage: node scripts/import-taca-1974-1975.mjs
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { BATCHES } from "./data/seasons-taca-1974-1975.mjs";
...
const CREATE_META = {
  tuca: { position: "Atacante", forceCreate: true },
};
...
async function ensureStadium(name) {
  ...
  const aliases = {
    ...
    independencia: (s) => norm(s.name).includes("independencia"),
    "pedro ludovico": (s) => norm(s.name).includes("ludovico"),
    "vila belmiro": (s) => norm(s.name).includes("belmiro"),
    araripe: (s) => norm(s.name).includes("araripe"),
    godofredo: (s) => norm(s.name).includes("godofredo"),
...
try {
  ...
  console.log("OK Taça de Ouro 1974–1975");
} catch (e) {
