/**
 * Import Campeonato Alagoano 1971 (26 jogos CSA; decisão em mar/1973).
 * - CSA campeão (TJD; decisão 15–25/03/1973)
 * - Técnicos: Hélio Miranda (a partir de 09/06); Wassil Barbosa (decisão)
 * - Oficial: J26 V15 E6 D5 GP51 GC25
 * - Zé Luiz goleiro = #1783 (mesmo de 1972; ≠ década de 80)
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1971-alagoano.mjs";
...
const CANON = {
  ...
  "ze luiz i": "Zé Luiz I",
  "zé luiz i": "Zé Luiz I",
  "ze raimundo": "Zé Raimundo",
  "zé raimundo": "Zé Raimundo",
  soares: "Soareste",
  soareste: "Soareste",
  otavio: "Otávio",
  otávio: "Otávio",
...
const FORCE_ID = {
  ...
  dudu: 1777,
  beto: 1775,
  "fernando carlos": 1779,
  valter: 1781,
  ricardo: 1780,
  // Goleiro 1971/72 (#1783) — não confundir com #1180/#1689 da década de 80
  "ze luiz": 1783,
  "zé luiz": 1783,
  "ze luiz i": 1181,
  "zé luiz i": 1181,
...
const CREATE_META = {
  zito: { position: "Atacante", forceCreate: true },
  caroco: { position: "Atacante", forceCreate: true },
  caroço: { position: "Atacante", forceCreate: true },
  zeze: { position: "Atacante", forceCreate: true },
  zezé: { position: "Atacante", forceCreate: true },
  "ze raimundo": { position: "Volante", forceCreate: true },
  "zé raimundo": { position: "Volante", forceCreate: true },
  "jose maria": { position: "Atacante", forceCreate: true },
  "josé maria": { position: "Atacante", forceCreate: true },
  tenorio: { position: "Atacante", forceCreate: true },
  tenório: { position: "Atacante", forceCreate: true },
};
...
async function ensureManager(name) {
  ...
  if (norm(canon).includes("wassil") || norm(canon) === "barbosa") {
    const v = all.find((m) => norm(m.name).includes("wassil barbosa"));
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log("oficial: J26 V15 E6 D5 GP51 GC25 | CSA campeão 1971");
  console.log("OK Alagoano 1971");
} catch (e) {
