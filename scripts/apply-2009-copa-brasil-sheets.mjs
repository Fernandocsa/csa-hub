/**
 * Apply complementary Copa do Brasil 2009 sheets (CSA-only).
 * Usage: node scripts/apply-2009-copa-brasil-sheets.mjs [--dry]
 */
...
import {
  ...
  PHASE_BY_DATE,
  SHEETS,
} from "./data/season-2009-copa-brasil-sheets.mjs";
...
const FORCE_ID = {
  ...
  "anderson alagoano": 2105,
  "anderson canhao": 2105,
  augusto: 2038,
  "cleisson rato": 2021,
  camilo: 2045,
  "marco brito": 2051,
  "thiago potiguar": 2031,
  "tiago potiguar": 2031,
  "fabio lopes": 2037,
  "ricardo miranda": 2008,
  leandro: 2052,
};

const CREATE_META = {
  rodrigues: { name: "Rodrigues", position: "Goleiro", forceNew: true },
  "tiago laranjeira": {
    name: "Tiago Laranjeira",
    position: "Atacante",
    forceNew: true,
  },
  "jonatas vieira": {
...
const STADIUM_ALIAS = {
  ...
  "estadio vila belmiro": "Estádio Vila Belmiro",
  "vila belmiro": "Estádio Vila Belmiro",
  "estadio couto pereira": "Estádio Couto Pereira",
  "couto pereira": "Estádio Couto Pereira",
  "estadio estiva": "Estádio Estiva",
  estiva: "Estádio Estiva",
};
...
try {
  ...
  for (const m of allMatches) {
    ...
    for (const red of sheet.csaReds ?? []) {
      const name = typeof red === "string" ? red : red.name;
      const minute = typeof red === "string" ? 0 : (red.minute ?? 0);
      let p = null;
      try {
    ...
    for (const red of sheet.oppReds ?? []) {
      const name = typeof red === "string" ? red : red.name;
      const minute = typeof red === "string" ? 0 : (red.minute ?? 0);
      if (!DRY) {
        await client.query(
          `INSERT INTO match_cards
             (match_id, side, card_type, lineup_id, player_id, player_name, minute, injury_time_minute)
           VALUES ($1,'opponent','red',NULL,NULL,$2,$3,NULL)`,
          [m.id, name, minute],
        );
      }
    }
