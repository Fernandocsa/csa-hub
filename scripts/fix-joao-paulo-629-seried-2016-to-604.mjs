/**
 * Move Série D 2016 unused-bench sheets from #629 João Paulo Penha
 * → #604 João Paulo (goleiro).
 *
 * Usage: node scripts/fix-joao-paulo-629-seried-2016-to-604.mjs [--dry]
 */
...
const FROM = 629;
const TO = 604;
const TO_NAME = "João Paulo";
const SEASON = "2016";
const COMP_ID = 8; // Série D
...
try {
  // Only unused bench in Série D 2016 (GK related; Penha never entered those)
  const rows = (
    await c.query(
      `
  ...
  if (entered.length) {
    throw new Error(
      `unexpected entered benches on #629 Serie D: ${entered.map((r) => r.id).join(",")}`,
    );
  }
  ...
  for (const r of rows) {
    console.log(`  #${r.id} ${String(r.d).slice(0, 15)} vs ${r.opp} — banco (não entrou)`);
  }
  ...
  if (DRY) {
    ...
    console.log(
      ...
      (
        await c.query(
          `
          SELECT m.id, m.match_date::date d, o.name opp, l.role
          FROM match_lineups l
          JOIN matches m ON m.id=l.match_id
          JOIN opponents o ON o.id=m.opponent_id
          WHERE l.player_id=$1 ORDER BY m.match_date
          `,
          [TO],
        )
      ).rows,
