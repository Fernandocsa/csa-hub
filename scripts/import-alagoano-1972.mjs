/**
 * Import Campeonato Alagoano 1972 (32 jogos; 31 na contagem oficial).
 * - 04/06 vs São Domingos: jogo anulado (falta de energia); remarcado 07/06
 * - Técnicos: Pedrinho Rodrigues; Jorge Vasconcelos (fim do Supercampeonato)
 * - Zé Luiz goleiro: cadastro novo (≠ #1180/#1689 da década de 80)
 * - Oficial: J31 V16 E9 D6 GP57 GC34
 */
import { loadEnvFromDotenv, createPgPool } from "./_load-env.mjs";
import { COMPETITION_NAME, SEASON, GAMES } from "./data/season-1972-alagoano.mjs";
...
const CANON = {
  "pedrinho rodrigues": "Pedrinho Rodrigues",
  ...
  "manoelzinho i": "Manoelzinho I",
  "manoelzinho ii": "Manoelzinho II",
  mario: "Mário",
  mário: "Mário",
};
...
const CREATE_META = {
  // Goleiro de 1972 — NÃO reutilizar #1180/#1689/#1686 da década de 80
  "ze luiz": { position: "Goleiro", forceCreate: true },
  "zé luiz": { position: "Goleiro", forceCreate: true },
  "manoelzinho i": { position: "Atacante", forceCreate: true },
  "manoelzinho ii": { position: "Atacante", forceCreate: true },
  "fernando carlos": { position: "Atacante", forceCreate: true },
  teco: { position: "Lateral", forceCreate: true },
  valter: { position: "Volante", forceCreate: true },
...
async function ensureManager(name) {
  ...
  if (norm(canon).includes("pedrinho")) {
    const v = all.find((m) => norm(m.name).includes("pedrinho"));
    if (v) return v;
  }
  const ins = await client.query(
...
try {
  ...
  console.log("oficial (exclui anulado 04/06): J31 V16 E9 D6 GP57 GC34");
  console.log("OK Alagoano 1972");
} catch (e) {
