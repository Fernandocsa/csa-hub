...
import { useGetOpponentsByState } from "@workspace/api-client-react";
...
export default function MatchesByState() {
  const { data, isLoading, isError } = useGetOpponentsByState();
  ...
  const totalOpponents =
    states.reduce((s, r) => s + r.opponentCount, 0) + (unknown?.opponentCount ?? 0);

  return (
    <div className="space-y-5 max-w-4xl">
      <div className="border-b pb-3">
        <h1 className="text-xl font-bold" data-testid="heading-csa-x-estados">
          CSA x Estados
        </h1>
        <p className="text-sm text-muted-foreground">
          Histórico agregado do CSA contra times de cada estado brasileiro
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p>Nenhum adversário com UF cadastrada ainda.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {states.map((row) => (
            <Link
              ...
              data-testid={`link-state-${row.state}`}
            >
          ...
          {unknown && (
            <Link
              href="/partidas/por-estado/sem-estado"
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {unknown.opponentCount} time{unknown.opponentCount === 1 ? "" : "s"} sem UF
              </p>
              <div className="flex flex-wrap gap-3 mt-3 text-sm">
